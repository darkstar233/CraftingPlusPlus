...
# 是否允许交易机使用创造模式
scoreboard players set $doTradeCreate cppConfig 1
# 生物投影仪工作周期
...
# 空岛模式
execute unless block ~ 0 ~ bedrock run scoreboard players set $skyislandMode cppConfig 1
execute if block ~ 0 ~ bedrock run scoreboard players set $skyislandMode cppConfig 0
