tag @e[type=area_effect_cloud,distance=..8,tag=cpp_locate_block,tag=cpp_temp] remove cpp_temp
# 计算三交点参数
...
# 存储交点
summon area_effect_cloud ~ ~ ~ {Tags:["cpp_locate_block","cpp_temp"]}
execute if score #sf cppValue matches 0.. store result entity @e[type=area_effect_cloud,distance=..9,tag=cpp_locate_block,tag=cpp_temp,limit=1] Pos[0] double 0.01 run scoreboard players get #x cppValue
execute if score #sg cppValue matches 0.. store result entity @e[type=area_effect_cloud,distance=..9,tag=cpp_locate_block,tag=cpp_temp,limit=1] Pos[1] double 0.01 run scoreboard players get #y cppValue
execute if score #sh cppValue matches 0.. store result entity @e[type=area_effect_cloud,distance=..9,tag=cpp_locate_block,tag=cpp_temp,limit=1] Pos[2] double 0.01 run scoreboard players get #z cppValue
# 根据朝向符号调整坐标
execute if score #sf cppValue matches ..-1 store result entity @e[type=area_effect_cloud,distance=..9,tag=cpp_locate_block,tag=cpp_temp,limit=1] Pos[0] double -0.01 run scoreboard players get #x cppValue
execute if score #sg cppValue matches ..-1 store result entity @e[type=area_effect_cloud,distance=..9,tag=cpp_locate_block,tag=cpp_temp,limit=1] Pos[1] double -0.01 run scoreboard players get #y cppValue
execute if score #sh cppValue matches ..-1 store result entity @e[type=area_effect_cloud,distance=..9,tag=cpp_locate_block,tag=cpp_temp,limit=1] Pos[2] double -0.01 run scoreboard players get #z cppValue
# 若未找到，循环
execute at @e[type=area_effect_cloud,distance=..8,tag=cpp_locate_block,tag=cpp_temp] if entity @s[distance=..9] if block ~ ~ ~ #cpp:fluid run function cpp:locate/air_loop
