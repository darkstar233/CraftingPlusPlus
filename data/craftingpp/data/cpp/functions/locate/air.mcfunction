...
# 获取与方块交点
function cpp:locate/air_loop
execute as @e[type=area_effect_cloud,distance=..8,tag=cpp_locate_block,tag=cpp_temp] at @s if block ~ ~ ~ #cpp:fluid run kill @e[type=area_effect_cloud,distance=..8,tag=cpp_locate_block]
kill @e[type=area_effect_cloud,distance=..8,tag=cpp_locate_block,tag=cpp_temp]
tag @e[type=area_effect_cloud,distance=..8,tag=cpp_locate_block,sort=furthest,limit=1] add cpp_front_air
